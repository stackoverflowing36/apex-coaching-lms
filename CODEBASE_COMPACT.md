# Apex LMS — Compact Architecture & Code Reference for Claude Code

> **Token-Optimized Edition**: Designed specifically for low token consumption when working with Claude Code while preserving 100% of architecture, types, queries, schema, and page logic.

## 1. System Architecture & Tech Stack
- **Framework**: Next.js 14 (App Router)
- **Database & Storage**: Supabase (PostgreSQL, Auth, Storage bucket `course-materials`)
- **Key Mechanism 1 (Chapter Fallback)**: In case PostgREST schema cache misses `chapter_id` on tables, chapters and item mappings are persisted to and synchronized from `course-materials/{courseId}/_chapters_metadata.json`.
- **Key Mechanism 2 (Lecture Numbering & Deletion)**: Lectures are numbered sequentially (`1, 2, 3...`) inside green circular badges. Deletion performs optimistic state updates and runs `reorderLectures(supabase, updates)` to keep sequential ordering without gaps.
- **Key Mechanism 3 (Dual Portal)**: Student Portal (`/student/*`) and Teacher Portal (`/teacher/*`).
...
## 3. Data Access Layer (`src/lib/supabase/queries.ts` - Essential Code)
```typescript
...
## 5. Application Route Map & Specifications
### Student Portal Routes
1. **`/student/dashboard`** (`src/app/student/dashboard/page.tsx`):
   - Metrics: Total lectures, pending assignments, upcoming tests.
   - Announcements feed with batch tags.
   - Upcoming deadlines list with chapter badges and urgency tags.
2. **`/student/lectures`** (`src/app/student/lectures/page.tsx`):
   - **Tab 1: Video Lectures**: Grouped by course batch. Each lecture features a green circular numbering badge (`#1, #2...`), batch code, chapter badge, title, and direct link to video player.
   - **Tab 2: PDF Notes / Materials**: Grid of downloadable course materials, formula sheets, and PDFs with chapter badges and download buttons.
   - **Tab 3: Practice Quizzes**: Filterable quiz cards with time limit, total marks, chapter badge, and student's prior score if attempted.
3. **`/student/lectures/[lectureId]`** (`src/app/student/lectures/[lectureId]/page.tsx`):
   - Embeds YouTube, Vimeo, Google Drive, or Supabase Storage MP4 videos.
   - Header with green circle numbering badge, course code, chapter badge, and lecture notes PDF download link.
   - Sticky syllabus sidebar showing all sibling lectures in the course with circular green numbering and chapter tags.
4. **`/student/assignments`** (`src/app/student/assignments/page.tsx`):
   - Filterable assignments list with batch pills, max marks, chapter badges, due dates, and status badges (Pending, Submitted, Graded).
5. **`/student/assignments/[assignmentId]`** (`src/app/student/assignments/[assignmentId]/page.tsx`):
   - Detailed assignment instructions, attachments, submission dropzone, and evaluation view with teacher annotations & checked PDF download.
6. **`/student/quizzes/[quizId]`** (`src/app/student/quizzes/[quizId]/page.tsx`):
   - Interactive MCQ quiz interface with timer, multiple attempt support, question navigation, score calculation, and submission. Includes chapter badges.
7. **`/student/grades`** (`src/app/student/grades/page.tsx`):
   - Detailed evaluation breakdown, teacher feedback, marks obtained, chapter badges, and student attendance rate.

### Teacher Portal Routes
1. **`/teacher/dashboard`** (`src/app/teacher/dashboard/page.tsx`):
   - Overview metrics, quick actions, batch summary, pending evaluations.
2. **`/teacher/courses`** (`src/app/teacher/courses/page.tsx`):
   - Batch creation and listing.
3. **`/teacher/courses/[courseId]`** (`src/app/teacher/courses/[courseId]/page.tsx`):
   - **Lectures Manager**: Move up/down, add lecture modal with `ChapterSelect`, green circle numbering (`idx + 1`), and delete lecture handler that reorders remaining lectures sequentially in DB.
   - **Study Materials Tab**: Upload PDF with chapter selection and preview.
   - **Assignments Tab**: Create assignments with due dates, attachments, and chapter assignment.
   - **Classroom Students**: Enrolled student management.
4. **`/teacher/quizzes`** (`src/app/teacher/quizzes/page.tsx`):
   - Create custom quizzes with MCQ questions, rich question formatting, time limits, and chapter tags.
5. **`/teacher/grading`** & **`/teacher/grading/[submissionId]`**:
   - Digital homework evaluation suite with handwriting/pen annotations, grading tools, and merged PDF export.
6. **`/teacher/attendance`** (`src/app/teacher/attendance/page.tsx`):
   - Daily batch attendance marking (Present/Absent/Late).
7. **`/teacher/announcements`** (`src/app/teacher/announcements/page.tsx`):
